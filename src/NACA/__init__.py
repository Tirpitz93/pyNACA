...
from math import floor, rem, sin, cos, tan


def four_digit(n: [str, int], alpha=0, c=1, s=1000, cs=1, cte=0):
    m = floor(n / 1000) / 100
    ...
    for i in arange(1, s, 1).reshape(-1):
        ...
        # matlab_src/naca.m:98
            ...
            y_c[i] = dot(dot(m, (1 - x[i])) / (1 - p) ** 2, (1 + x[i] - dot(2, p))) + dot((1 / 2 - x[i]),
                                                                                          sin(alpha))
...
def NACA(n=None, alpha=None, c=None, s=None, cs=None, cte=None, *args, **kwargs):
    ...
    if exist('cte', 'var'):
        if cte == 1:
            y_t = dot(t / 0.2, (
                    dot(0.2969, sqrt(x)) - 0.126 * x - 0.3516 * x ** 2 + 0.2843 * x ** 3 - 0.1036 *
                    x ** 4))
        # matlab_src/naca.m:75
        else:
            y_t = dot(t / 0.2, (
                    dot(0.2969, sqrt(x)) - 0.126 * x - 0.3516 * x ** 2 + 0.2843 * x ** 3 - 0.1015 *
                    x ** 4))
        ...
        if nc == 5:
            ...
            if rn == 0:
                ...
                for i in arange(1, s, 1).reshape(-1):
                    if x[i] < r:
                        y_c[i] = dot(k1 / 6,
                                     (x[i] ** 3 - dot(dot(3, r), x[i] ** 2) + dot(dot(r ** 2, (3 - r)), x[i]))) + dot(
                                (1 / 2 - x[i]), sin(alpha))
                        # matlab_src/naca.m:117
                        dyc_dx[i] = dot(k1 / 6,
                                        (dot(3, x[i] ** 2) - dot(dot(6, r), x[i]) + dot(r ** 2, (3 - r)))) / cos(
                                alpha) - tan(alpha)
            ...
            # matlab_src/naca.m:121
                ...
                if rn == 1:
                    ...
                    k1 = dot(- 27973.3333333385,
                             p ** 3) + 17972.8000000027 * p ** 2 - 3888.40666666711 * p + 289.076000000022
                    ...
                    for i in arange(1, s, 1).reshape(-1):
                        if x[i] < r:
                            y_c[i] = dot(k1 / 6, ((x[i] - r) ** 3 - dot(dot(k2_k1, (1 - r) ** 3), x[i]) - dot(r ** 3,
                                                                                                              x[i]) + r ** 3)) + dot(
                                    (1 / 2 - x[i]), sin(alpha))
                            # matlab_src/naca.m:133
                            dyc_dx[i] = dot(k1 / 6,
                                            (dot(3, (x[i] - r) ** 2) - dot(k2_k1, (1 - r) ** 3) - r ** 3)) / cos(
                                    alpha) - tan(alpha)
                        # matlab_src/naca.m:134
                        else:
                            y_c[i] = dot(k1 / 6, (
                                    dot(k2_k1, (x[i] - r) ** 3) - dot(dot(k2_k1, (1 - r) ** 3), x[i]) - dot(r ** 3,
                                                                                                            x[i]) + r ** 3)) + dot(
                                    (1 / 2 - x[i]), sin(alpha))
                            # matlab_src/naca.m:136
                            dyc_dx[i] = dot(k1 / 6, (
                                    dot(dot(3, k2_k1), (x[i] - r) ** 2) - dot(k2_k1, (1 - r) ** 3) - r ** 3)) / cos(
                                    alpha) - tan(alpha)
                # matlab_src/naca.m:137
                else:
                    error('Incorrect NACA number. Third digit must be either 0 or 1')
        else:
            if nc == 6:
                ...
                if ser == 6:
                    # ----------------------- CAMBER ---------------------------------------
                    y_c = dot(c_li / (dot(dot(2, pi), (a + 1))), (dot(1 / (1 - a), (
                            dot(dot(1 / 2, (a - x) ** 2.0), log(abs(a - x))) - dot(dot(1 / 2, (1 - x) ** 2.0),
                                                                                   log(1 - x)) + dot(1 / 4, (
                            1 - x) ** 2) - dot(1 / 4, (a - x) ** 2))) - multiply(x, log(x)) + g - dot(h,
                                                                                                      x))) + dot(
                            (1 / 2 - x), sin(alpha))
                    # matlab_src/naca.m:153
                    dyc_dx = - (dot(c_li, (h + log(x) - (x / 2 - a / 2 + (multiply(log(1 - x), (dot(2, x) - 2))) / 2 + (
                            multiply(log(abs(a - x)), (dot(2, a) - dot(2, x)))) / 2 + (
                                                                 multiply(sign(a - x), (a - x) ** 2)) / (
                                                                 dot(2, abs(a - x)))) / (a - 1) + 1))) / (
                                     dot(dot(dot(2, pi), (a + 1)), cos(alpha))) - tan(alpha)
                # matlab_src/naca.m:154
                else:
                    error('NACA 6 Series must begin with 6')
            else:
                error(concat(['NACA ', num2str(nc), ' Series has not been yet implemented']))
    ...
    clp = plot(concat([dot(c / 2, (1 - cos(alpha))), dot(c / 2, (1 + cos(alpha)))]),
               concat([dot(c / 2, sin(alpha)), dot(- c / 2, sin(alpha))]), 'g')
    ...
    if sym == 1:
        title(concat(['NACA 00', num2str(n), ' Airfoil Plot (', num2str(dot(alpha / pi, 180)), 'º)']))
        legend(concat([ep, mclp, clp]), concat(['NACA 00', num2str(n), ' Airfoil']), 'Mean camber line', 'Chord line')
    else:
        if sym == 2:
            title(concat(['NACA 0', num2str(n), ' Airfoil Plot (', num2str(dot(alpha / pi, 180)), 'º)']))
            legend(concat([ep, mclp, clp]), concat(['NACA 0', num2str(n), ' Airfoil']), 'Mean camber line',
                   'Chord line')
        else:
            title(concat(['NACA ', num2str(n), ' Airfoil Plot (', num2str(dot(alpha / pi, 180)), 'º)']))
            legend(concat([ep, mclp, clp]), concat(['NACA ', num2str(n), ' Airfoil']), 'Mean camber line', 'Chord line')
